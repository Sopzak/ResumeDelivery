...
public class ThirdPersonOrbitCamBasic : MonoBehaviour 
{
	...
	void Update()
	{
		...
		//	angleV += Mathf.Clamp(Input.GetAxis("Mouse Y"), -1, 1) * verticalAimingSpeed;
			...
			// Joystick:
			angleH += Mathf.Clamp(SimpleInput.GetAxis(XAxis), -1, 1) * 10 * horizontalAimingSpeed * Time.deltaTime;
			angleV += Mathf.Clamp(SimpleInput.GetAxis(YAxis), -1, 1) * 10 * verticalAimingSpeed * Time.deltaTime;
		//#endif
		//#if (UNITY_ANDROID)
			//Player.GetComponent<MouseLookScript> ().LookAxis = TouchField.TouchDist;
		//	angleH += Mathf.Clamp(TouchField.TouchDist.x, -1, 1) * 60 * horizontalAimingSpeed * Time.deltaTime;
		//	angleV += Mathf.Clamp(TouchField.TouchDist.y, -1, 1) * 60 * verticalAimingSpeed * Time.deltaTime;
		//#endif
